...
export const ATTACHMENT_SCAN_LIMIT = 20

export async function listLetterAttachments(ctx: LetterCtx, letterId: Id<'letters'>) {
  return await ctx.db
    .query('letterAttachments')
    .withIndex('by_letterId', (q) => q.eq('letterId', letterId))
    .take(ATTACHMENT_SCAN_LIMIT)
}

export function attachmentsAreReadyForSend(
  attachments: Array<{ status: Doc<'letterAttachments'>['status'] }>,
): boolean {
  return (
    attachments.length < ATTACHMENT_SCAN_LIMIT &&
...
export async function sendOwnedLetter(
  ...
  if (!attachmentsAreReadyForSend(attachments)) {
