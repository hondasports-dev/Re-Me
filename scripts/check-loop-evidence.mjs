...
function validateLearningCandidate(candidate, index) {
  ...
  if (!isNonEmptyList(axes) || axes.some((axis) => !LEARNING_IMPROVEMENT_AXES.has(axis))) {
    errors.push(`candidate[${index}].improvement_axes must contain only context, speed, or precision`);
  }
  ...
  if (disposition === "follow_up") {
    ...
    if (
      ...
      errors.push(`candidate[${index}].persistent_follow_up requires type issue/task/pr and reference`);
    }
...
export function evaluateLearningApplication({ userRequestedCurrentPrApply = false, learning }) {
  ...
  if (candidates.length > 3) errors.push("learning candidates must be limited to the 3 highest-impact items");
...
export function evaluateVerificationEvidence({ evidence }) {
  ...
  if (!Array.isArray(evidence.reruns)) {
    ...
    for (const [index, rerun] of evidence.reruns.entries()) {
      if (isEmptyText(rerun?.check) || isEmptyText(rerun?.reason) || isEmptyText(rerun?.invalidated_by)) {
...
export function evaluateReviewEvidence({ evidence, headSha = "" }) {
  ...
  if (!required) {
    if (status !== "NOT_REQUIRED") errors.push("review.status must be NOT_REQUIRED when review is not required");
...
export function parseLoopEvidenceArguments(args) {
  ...
  if (!options.file && !options.evidenceJson) throw new Error("--file or --evidence-json is required");
  return options;
}
