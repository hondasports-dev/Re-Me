...
export function evaluateWorkspaceState({
  ...
  if (requireClean && dirty) {
    errors.push("baseline worktree is dirty; resolve or explicitly own pre-existing changes before editing");
  }

  return { ok: errors.length === 0, errors, baseline: dirty ? "DIRTY" : "CLEAN" };
}
...
function printResult({ result, branch, currentPath, canonicalPath, scope }) {
  ...
  console.log(`current_worktree: ${currentPath === null ? "(not checked)" : currentPath || "(unknown)"}`);
  console.log(`canonical_worktree: ${canonicalPath === null ? "(not checked)" : canonicalPath || "(unknown)"}`);
  console.log(`baseline: ${result.baseline}`);
  for (const error of result.errors) console.error(`error: ${error}`);
}

export function runWorkspacePreflight({ cwd = process.cwd(), requireClean = false, staged = false } = {}) {
  try {
    if (staged) {
    ...
    const registered = entries.some((entry) => normalizePath(entry.path) === normalizePath(currentPath));
