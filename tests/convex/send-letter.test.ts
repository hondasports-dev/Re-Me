...
describe('sendLetter', () => {
  ...
  it('rejects empty body, missing delivery mode, and unfinished attachments', async () => {
    ...
    await expect(
      asAlice.query(api.letters.getDraft, { letterId: pending.letterId }),
    ).resolves.toMatchObject({ attachmentsReady: false })
    ...
    await expect(
      asAlice.query(api.letters.getDraft, { letterId: deleting.letterId }),
    ).resolves.toMatchObject({ attachmentsReady: false })

    const overflow = await prepareSendableDraft(asAlice)
    const overflowOwner = (await asAlice.mutation(api.users.ensureCurrentUser, {})).userId
    await t.run(async (ctx) => {
      const now = Date.now()
      for (let index = 0; index < 20; index += 1) {
        await ctx.db.insert('letterAttachments', {
          letterId: overflow.letterId,
          ownerId: overflowOwner,
          kind: 'photo',
          status: 'ready',
          generationToken: `overflow-${index}`,
          createdAt: now,
        })
      }
    })
    await expect(
      asAlice.mutation(api.letters.sendLetter, { letterId: overflow.letterId }),
    ).rejects.toThrow(/attachments are not ready/)
    await expect(
      asAlice.query(api.letters.getDraft, { letterId: overflow.letterId }),
    ).resolves.toMatchObject({ attachmentsReady: false })
  })
